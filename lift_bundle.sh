...
RC_METHOD="${BUILD_METHOD:-ADAPT-NEW-STRUCTURE}"
...
[11] HOOK-IMAGE POWER TEST (v598) — THIS IS THE BIGGEST VIRAL LEVER.
     ...
     evidence trigger is present — i.e. the decoded source, or the
     approved direction, is a SPECTACLE-PROP hook (a manipulated
     symptom / proxy object). Outside that family they are a
     comparison lens, NOT a gate: proven hooks carry the glance on a
     burned-in numeral pair, a caption card, a montage or a keyed
     reaction face with no prop in frame one. A lift NEVER bolts a
     prop onto a source whose hook did not have one — that breaks
     source fidelity. Canonical: template_reference.md §v598
