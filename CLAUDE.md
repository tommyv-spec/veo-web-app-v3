...
- **`autoedit_*` CODE HAS TWO DEPLOY TARGETS — `git push` reaches ONE (2026-08-27).** The auto-edit runs on a LOCAL worker that keeps its **own copy** of `autoedit_pipeline.py` · `autoedit_qc.py` · `autoedit_captions.py` · `autoedit_queue.py` · `audio_processor.py` · `config.py` · the caption templates, laid down at install time under `%USERPROFILE%\veo-worker\autoedit\` from `/api/user-worker/download/autoedit/<name>`. Push to Render, **then refresh the worker's copy through that same route and restart the worker** (`start-autoedit.bat`, or python directly with `.env` loaded). A server-only change does not fail loudly — it fails inside the worker as an ordinary job failure; v948.2's first run died three times on `ValueError: audio_enhance must be 'voice' or 'off'` raised from the worker's stale `autoedit_qc.py` while the server had the new code. **Diff with `diff --strip-trailing-cr`** — the installed copies are CRLF, so a plain diff reports whole-file differences on files that are actually identical, and you cannot see which ones are genuinely stale. Adding a NEW file (e.g. a second caption template) also needs it on the server's `ALLOWED` map in `serve_autoedit_worker_file`, or the worker cannot fetch it at all. Canonical: `code/template_reference.md` §v948.2.
