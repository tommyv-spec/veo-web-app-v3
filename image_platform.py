...
def _invalidate_generation_outputs(db: Session, node: "ImageNode") -> None:
    """Drop generated bytes and lane proof after any render-contract edit."""
    _clear_choice(node)
    _clear_qc(node)
    node.status = "draft"
    node.cg_status = None
    ...
    for variant in list(node.variants or []):
        if (getattr(variant, "source", "ai") or "ai") == "manual":
            ...
            continue
        try:
            _storage_delete(variant.image_path)
            path = images_root() / variant.image_path
            if path.exists():
                path.unlink()
            for thumb_rel in _thumb_rels_for(variant.image_path):
            ...
            log.warning(
                f"[image_platform] contract invalidation could not remove "
                f"variant {variant.id}: {exc}"
            )
        db.delete(variant)
...
def update_node(
    ...
    if contract_changed:
        _invalidate_generation_outputs(db, node)
    node.updated_at = datetime.utcnow()
    db.commit()
    db.refresh(node)
    return node.to_dict()
...
def _promote_ready_children(db: Session, parent_node_id: int):
    ...
    for child in children:
        if child.status != "draft":
            ...
            if not stale:
                continue
            _invalidate_generation_outputs(db, child)
