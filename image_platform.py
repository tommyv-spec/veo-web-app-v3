...
def run_image_platform_migrations():
    ...
    sqlite_migrations = [
        ...
        ("image_nodes", "completed_contract_hash",
         "ALTER TABLE image_nodes ADD COLUMN completed_contract_hash TEXT"),
        ("image_nodes", "cg_completed_contract_hash",
         "ALTER TABLE image_nodes ADD COLUMN cg_completed_contract_hash TEXT"),
    ...
    postgres_migrations = [
        ...
        ("image_nodes", "completed_contract_hash",
         "ALTER TABLE image_nodes ADD COLUMN IF NOT EXISTS completed_contract_hash VARCHAR(64)"),
        ("image_nodes", "cg_completed_contract_hash",
         "ALTER TABLE image_nodes ADD COLUMN IF NOT EXISTS cg_completed_contract_hash VARCHAR(64)"),
        ("image_nodes", "batch_id",
...
# v959 — the mentor's clip method inside the platform. Declared per scene:
#   - **render_method:** movie-section
#   - **face_refs:** image_K[, image_L]      (1-2 face reference images)
#   - **clip_duration_s:** 8 | 10            (the section WINDOW, mandatory)
# One scene = one Ingredients clip = one line carrying every spoken word.
MOVIE_SECTION_RENDER_METHOD = "movie-section"
...
def _invalidate_generation_outputs(
    db: Session, node: "ImageNode",
) -> List[str]:
    """Mark generated outputs invalid and return files to clean after commit.

    Database rollback can restore variant rows, but it cannot restore local or
    object-storage bytes. Callers must commit these row changes before deleting
    any returned paths.
    """
    _clear_choice(node)
    _clear_qc(node)
    ...
    node.claimed_prompt_hash = None
    node.cg_claimed_prompt_hash = None
    node.completed_contract_hash = None
    node.cg_completed_contract_hash = None
    cleanup_paths: List[str] = []
    for variant in list(node.variants or []):
        if (getattr(variant, "source", "ai") or "ai") == "manual":
            _clear_verdict(variant)
            continue
        cleanup_paths.append(variant.image_path)
        db.delete(variant)
    return cleanup_paths


def _cleanup_invalidated_variant_files(image_paths: List[str]) -> None:
    """Best-effort cleanup after invalidation rows are durably committed."""
    for image_path in image_paths:
        try:
            _storage_delete(image_path)
            path = images_root() / image_path
            if path.exists():
                path.unlink()
            for thumb_rel in _thumb_rels_for(image_path):
                _storage_delete(thumb_rel)
                thumb = images_root() / thumb_rel
                if thumb.exists():
                    thumb.unlink()
        except Exception as exc:
            log.warning(
                f"[image_platform] committed contract invalidation could not "
                f"remove {image_path}: {exc}"
            )
...
class ImageNode(Base):
    ...
    # Full render-contract hashes stamped only after each lane saves output.
    # Unlike the short in-flight claim fingerprints, these are checked again
    # at choose and promote time so an old image cannot survive a model,
    # sizing or chosen-parent change.
    completed_contract_hash = Column(String(64), nullable=True)
    cg_completed_contract_hash = Column(String(64), nullable=True)
    ...
    def to_dict(self, include_variants: bool = True) -> Dict[str, Any]:
        ...
        data = {
            ...
            "completed_contract_hash": self.completed_contract_hash,
            "cg_completed_contract_hash": self.cg_completed_contract_hash,
...
def update_node(
    ...
    cleanup_paths: List[str] = []
    if contract_changed:
        cleanup_paths = _invalidate_generation_outputs(db, node)
    node.updated_at = datetime.utcnow()
    db.commit()
    _cleanup_invalidated_variant_files(cleanup_paths)
    db.refresh(node)
    return node.to_dict()
...
def _generation_contract_hash(
    db: Session, node: ImageNode, backend: str,
) -> str:
    """Hash every input that can change the pixels produced by one lane."""
    import hashlib

    backend = _norm_backend(backend)
    references = []
    edges = db.query(ImageEdge).filter(
        ImageEdge.child_node_id == node.id,
    ).order_by(ImageEdge.slot_order.asc(), ImageEdge.id.asc()).all()
    for edge in edges:
        parent = db.query(ImageNode).filter(
            ImageNode.id == edge.parent_node_id,
        ).first()
        chosen_id = getattr(parent, "chosen_variant_id", None) if parent else None
        chosen = None
        if chosen_id is not None:
            chosen = db.query(ImageVariant).filter(
                ImageVariant.id == chosen_id,
                ImageVariant.node_id == edge.parent_node_id,
            ).first()
        references.append({
            "edge": {
                "parent_node_id": int(edge.parent_node_id),
                "role": edge.role or "",
                "slot_order": int(edge.slot_order or 0),
                "kind": (edge.kind or "").strip().lower(),
                "instruction": (edge.reference_instruction or "").strip(),
                "origin": (edge.origin or "manual").strip().lower(),
            },
            "chosen_parent_image": {
                "variant_id": chosen_id,
                "image_path": getattr(chosen, "image_path", None),
                "source": getattr(chosen, "source", None),
                "backend": getattr(chosen, "backend", None),
            },
        })
    payload = {
        "version": 1,
        ...
        "model": "chatgpt" if backend == "chatgpt" else (node.model or ""),
        "dimensions": {
            "aspect_ratio": node.aspect_ratio or "",
            "resolution": node.resolution or "",
        },
        "variant_count": 1 if backend == "chatgpt" else int(node.n_variants or 1),
...
def _completed_contract_hash(node: ImageNode, backend: str) -> Optional[str]:
    return (
        getattr(node, "cg_completed_contract_hash", None)
        if _norm_backend(backend) == "chatgpt"
        else getattr(node, "completed_contract_hash", None)
    )


def _set_completed_contract_hash(
    node: ImageNode, backend: str, value: Optional[str],
) -> None:
    if _norm_backend(backend) == "chatgpt":
        node.cg_completed_contract_hash = value
    else:
        node.completed_contract_hash = value
...
def _dual_backend_output_problems(db: Session, node: ImageNode) -> List[str]:
    ...
    for backend, status in lane_statuses.items():
        if status != "ready":
            ...
            continue
        expected_contract = _generation_contract_hash(db, node, backend)
        completed_contract = _completed_contract_hash(node, backend)
        if completed_contract != expected_contract:
            problems.append(
                f"{backend} output contract is stale or unproven "
                f"(completed={completed_contract or 'none'}, "
                f"current={expected_contract})"
            )
            continue
        candidates = [
...
def _parse_scene_blocks_new(md_text: str, known_image_indexes: set) -> List[Dict[str, Any]]:
    ...
    for block in blocks:
        ...
        if _rm_raw:
            ...
            if _rm not in ("charswap", MOVIE_SECTION_RENDER_METHOD):
                raise ValueError(
                    f"Scene {scene_index}: render_method {_rm_raw!r} is not a "
                    f"known method (expected 'charswap' or 'movie-section', or "
                    f"leave the bullet out for the normal render path — see "
                    f"template_reference.md §v943 / §v959)"
                )
            render_method = _rm
        ...
        # v959 — the swap bullets belong to charswap alone. A movie-section
        # scene renders from images, so it has no source video to swap into.
        if render_method == MOVIE_SECTION_RENDER_METHOD and (swap_source_video or swap_mode):
            raise ValueError(
                f"Scene {scene_index}: render_method=movie-section does not take "
                f"swap_source_video / swap_mode (v959)"
            )
        if (render_method != MOVIE_SECTION_RENDER_METHOD
                and _swap_declared and len(_swap_declared) != 3):
        ...
        # v971 — one bullet, routed to the columns that already hold these
        # answers. No new column: `source-original` / `none` are what
        # swap_audio has always meant, and `scene:N` is what audio_from_scene
        # has always meant. A row in V971_AUDIO_SOURCES that needs a NEW
        # destination is the moment to add one, and not before.
        #
        # Forward-only: `source-original` and `none` keep exactly their v943.1
        ...
        if _v971:
            ...
            if _v971_src in ("source-original", "none"):
                swap_audio = _v971_src           # v943.1's column, same values
            elif _v971_src == "scene":
                # The mutual-exclusion rule lives inside the OLD bullet's parse
                # above, which runs earlier and never sees this spelling.
                # Re-check it here or the new syntax is a way around a rule the
                # old syntax obeys — a second spelling must inherit EVERY
                # refusal the first one carries, not just some.
                if voiceover_anchor_image is not None:
                    raise ValueError(
                        f"Scene {scene_index}: declares both `audio: "
                        f"scene:{_v971_arg}` and voiceover_anchor_image="
                        f"image_{voiceover_anchor_image} — an anchor means "
                        f"'mint an audio twin' and borrowing a scene's audio "
                        f"means 'do not' (v971, same rule as v698A)"
                    )
                if (audio_from_scene is not None
                        and audio_from_scene != _v971_arg):
                    raise ValueError(
                        f"Scene {scene_index}: `audio: scene:{_v971_arg}` and "
                        f"`audio_from_scene: {audio_from_scene}` disagree — "
                        f"declare one (v971)"
                    )
                audio_from_scene = _v971_arg
            # `render` writes nothing: it IS the absence of an override, and
            # storing a sentinel for it would make every downstream reader
            # learn a new value to mean what NULL already means.
        ...
        if _fr_raw and render_method != MOVIE_SECTION_RENDER_METHOD:
            raise ValueError(
                f"Scene {scene_index}: `- **face_refs:**` only means something on "
                f"a render_method=movie-section scene (v959)"
            )
        if render_method == MOVIE_SECTION_RENDER_METHOD:
            if not _fr_raw:
                raise ValueError(
        ...
        # v959 — one section, one clip, one line. Same reason as v943: clip rows
        # are 1:1 with `- **line:**`, so a second line would fan the section into
        # two renders of the same prompt. The single line carries EVERY spoken
        # word of the section in order — it is what whisper aligns at export.
        if render_method == MOVIE_SECTION_RENDER_METHOD:
            if len(lines_list) != 1:
                raise ValueError(
        ...
        scenes.append({
            ...
            "render_method": render_method,          # None | 'charswap' | 'movie-section'
            ...
            "face_refs": face_refs,                  # v959 — [] on every non-section scene
        })
...
def _import_scene_table_impl(
    ...
    for img in images:
        ...
        if existing is not None:
            ...
            existing.cg_status = None
            existing.claimed_prompt_hash = None
            existing.cg_claimed_prompt_hash = None
            existing.completed_contract_hash = None
            existing.cg_completed_contract_hash = None
...
def _promote_ready_children(db: Session, parent_node_id: int):
    """Queue children whose parents are ready and invalidate stale renders.

    A different chosen parent image changes the child's generation contract.
    Existing output rows and lane states must be dropped before the child is
    queued again; otherwise a ready tile can display bytes from the old parent
    even though choose/promote correctly refuse them.
    ...
    for child in children:
        if child.status != "draft":
            ...
            lane_details = (
                (
                    "banana", child.status, child.claimed_prompt_hash,
                    any(_norm_backend(v.backend) == "banana" for v in child.variants),
                ),
                (
                    "chatgpt", child.cg_status, child.cg_claimed_prompt_hash,
                    any(_norm_backend(v.backend) == "chatgpt" for v in child.variants),
                ),
            )
            for backend, status, claimed_hash, has_output in lane_details:
                current = _generation_contract_hash(db, child, backend)
                completed = _completed_contract_hash(child, backend)
                if completed is not None and completed != current:
                    stale = True
                elif status == "ready" and has_output and completed is None:
                    stale = True
                if claimed_hash and claimed_hash != current[:16]:
                    stale = True
            if not stale:
                continue
            cleanup_paths = _invalidate_generation_outputs(db, child)
            # Make the row deletion durable while the child is still draft,
            # then remove bytes. A caller rollback or queue-write failure can
            # no longer resurrect rows whose files have already gone.
            db.commit()
            _cleanup_invalidated_variant_files(cleanup_paths)
...
def worker_get_pending_job(
    ...
    _v891_fp = _prompt_fingerprint(db, node, backend)
    if is_cg:
...
def _prompt_fingerprint(db: Session, node: ImageNode, backend: str) -> str:
    """Short in-flight fingerprint of the same full contract approval checks."""
    return _generation_contract_hash(db, node, backend)[:16]


def _requeue_lane_after_contract_change(node: ImageNode, backend: str) -> None:
    """Invalidate one lane without mutating the other lane's work."""
    if _norm_backend(backend) == "chatgpt":
        node.cg_status = "queued"
        node.cg_claimed_by = None
        node.cg_claimed_at = None
...
def worker_upload_variants(
    ...
    _current_fp = _prompt_fingerprint(db, node, backend)
    if _claimed_fp != _current_fp:
        log.info(
            f"[v891.4] node {node_id} backend={backend}: generation contract "
            f"changed since this job was claimed (claimed={_claimed_fp or 'none'} "
            f"now={_current_fp}) "
            f"— discarding {len(files or [])} stale render(s); the pending requeue "
            f"regenerates from the current contract"
        )
        _requeue_lane_after_contract_change(node, backend)
        db.commit()
        db.close()
        return {"ok": True, "superseded": True, "stale_contract": True,
    ...
    _set_completed_contract_hash(node, backend, None)
    ...
    with get_db() as db2:
        ...
        # Parent choice/model/size may have changed during the slow file/R2
        # phase. Bind this delivery to the claim again before inserting rows.
        _current_fp2 = _prompt_fingerprint(db2, node2, backend)
        if _claimed_fp != _current_fp2:
            for _idx, _filename, rel_str, target in pending_variants:
                try:
                    _storage_delete(rel_str)
                    if target.exists():
                        target.unlink()
                except Exception as exc:
                    log.warning(
                        f"[image_platform] stale-contract cleanup failed for "
                        f"{rel_str}: {exc}"
                    )
            _requeue_lane_after_contract_change(node2, backend)
            db2.commit()
            return {
                "ok": True,
                "superseded": True,
        ...
        for offset, (_worker_idx, filename, rel_str, target) in enumerate(pending_variants):
            ...
            db2.add(v)
            saved_count += 1
        _set_completed_contract_hash(
            node2,
            backend,
            _generation_contract_hash(db2, node2, backend),
        )
        db2.commit()
...
def worker_update_job_status(
    ...
    if req.status == "completed" and has_variants:
        current_contract = _generation_contract_hash(db, node, backend)
        if _completed_contract_hash(node, backend) != current_contract:
            _requeue_lane_after_contract_change(node, backend)
            node.updated_at = datetime.utcnow()
            db.commit()
            return {
