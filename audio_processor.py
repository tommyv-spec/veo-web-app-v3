...
def run_cmd(cmd: list, timeout: int = 300) -> tuple:
    ...
    try:
        p = subprocess.Popen(
            cmd,
            stdout=subprocess.DEVNULL,  # discard stdout — FFmpeg progress goes to stderr
...
def try_noisereduce(in_wav: Path, out_wav: Path, strength: float = 0.75) -> bool:
    ...
    try:
        ...
        # CRITICAL: trim output to exactly match input length.
        # STFT-based processing pads to FFT boundaries — output can be up to
        # n_fft samples longer than input (~43ms at 48kHz), causing A/V drift.
        if len(reduced) > len(audio):
...
def try_deepfilter_modal(in_wav: Path, out_wav: Path) -> bool:
    ...
    try:
        ...
        response = requests.post(
            modal_url,
            json={"audio_base64": audio_base64, "sample_rate": 48000},  # 48000 is correct - we extract at 48000
        ...
        # Decode output and trim to exact input length.
        # DeepFilterNet processes in frames — output can be slightly longer
        # than input, causing A/V drift when muxed back into video.
        ...
        try:
            ...
            # Load both to compare lengths
            in_audio, in_sr = _sf.read(str(in_wav))
            out_audio, out_sr = _sf.read(_io.BytesIO(output_bytes))
            
            # Trim to exact input length if longer
            if len(out_audio) > len(in_audio):
                out_audio = out_audio[:len(in_audio)]
                _sf.write(str(out_wav), out_audio.astype(_np.float32), out_sr)
            else:
                with open(out_wav, "wb") as f:
                    f.write(output_bytes)
        except Exception:
            # If trim fails, just write raw bytes
            with open(out_wav, "wb") as f:
                f.write(output_bytes)
...
def replace_audio(video_path: Path, audio_path: Path, output_path: Path) -> bool:
    """Replace audio in video with processed audio.

    Re-encodes BOTH streams so PTS resets to 0 — no A/V sync mismatch.
    Uses CRF 18 (visually lossless) instead of CRF 0 (mathematically lossless)
    to avoid OOM / timeout on resource-constrained hosts like Render.
    """
    cmd = [
        FFMPEG_BIN, "-y",
        "-i", str(video_path),
    ...
    # Validate output has both video and audio streams
    probe_cmd = [
    ...
    try:
        p = subprocess.run(probe_cmd, capture_output=True, text=True, timeout=30)
        if "video" not in p.stdout:
            logger.error(f"replace_audio: output has no video stream! ffprobe={p.stdout.strip()}")
            # Remove the broken file
            if output_path.exists():
                output_path.unlink()
            return False
    except Exception as e:
        logger.warning(f"replace_audio: could not validate output ({e}), proceeding anyway")
...
def enhance_audio(
    ...
    try:
        ...
        # THE ROOT CAUSE (confirmed by FFmpeg documentation and mailing lists):
        # When encoding AAC, FFmpeg's native encoder adds 1024 samples of priming
        # silence and writes an edit list (elst box) to signal "skip these samples".
        # When done in a TWO-PASS approach (extract WAV → filter → mux back):
        #   - The muxer in pass 2 writes an AAC edit list independently of the video
        #   - The stream-copied video has its own timing from the original encode
        #   - These two independently-created timing references don't align
        #
        # THE FIX: single-pass approach — apply audio filters DIRECTLY on the video
        # file in one FFmpeg command with -c:v copy + -af chain + -c:a aac.
        # When FFmpeg muxes both streams in the same pass, it has full context of
        # the video's PTS and writes the AAC edit list correctly relative to it.
        # Sync is guaranteed because both streams are created in the same mux session.
        
        # Build filter chain
        filters = []
        if remove_laughter:
            filters.append("arnndn=m=cb.rnnn")  # RNNoise, causal, zero-latency
        if apply_deepfilter:
            filters.append("afftdn=nf=-25:nr=33:tn=1")  # afftdn, causal
        if apply_voice_filter:
            filters.append(VOICE_FILTER)
        if apply_loudnorm:
            filters.append(f"loudnorm=I={LOUDNORM_I}:TP={LOUDNORM_TP}:LRA={LOUDNORM_LRA}:linear=false")
        
        if not filters:
            shutil.copy(str(video_path), str(output_path))
            return {"enhanced": False, "reason": "No filters enabled"}
        
        af_chain = ",".join(filters)
        print(f"[Audio Enhancement] Single-pass filter chain: {af_chain}", flush=True)
        
        if progress_callback:
            progress_callback("Enhancing audio...")
        
        # Single-pass: stream-copy video, filter+encode audio in same mux session
        cmd = [
            FFMPEG_BIN, "-y",
            "-i", str(video_path),
            "-c:v", "copy",
        ...
        # arnndn model file may not be present — retry with afftdn fallback
        if code != 0 and "arnndn" in af_chain:
            print("[Audio Enhancement] arnndn model not found, retrying with afftdn", flush=True)
            af_chain = af_chain.replace("arnndn=m=cb.rnnn", "afftdn=nf=-25")
            cmd[cmd.index("-af") + 1] = af_chain
            code, _, err = run_cmd(cmd)
        
        if code != 0:
            print(f"[Audio Enhancement] Failed: {err}", flush=True)
            shutil.copy(str(video_path), str(output_path))
            return {"enhanced": False, "reason": "Enhancement failed"}
        
        if remove_laughter:    stats["noisereduce_applied"] = True
        if apply_deepfilter:   stats["deepfilter_applied"] = True
        if apply_voice_filter: stats["voice_filter_applied"] = True
        if apply_loudnorm:     stats["loudnorm_applied"] = True
...
def enhance_audio_for_voice_clone(
    ...
    try:
        ...
        if not extract_audio(video_path, extracted_wav, sample_rate=48000):  # 48000 = DeepFilter native rate
