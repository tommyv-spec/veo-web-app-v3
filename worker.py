...
class JobWorker:
    ...
    def start(self):
        ...
        # Log Nano Banana key availability for continue mode enhancement
        nano_key = os.environ.get("NANO_BANANA_API_KEY", "").strip()
        gemini_keys = os.environ.get("GEMINI_API_KEYS", "").strip()
        if nano_key:
            print(f"[Worker] ✓ NANO_BANANA_API_KEY set (ending ...{nano_key[-6:]})", flush=True)
        elif gemini_keys:
            print(f"[Worker] ✓ GEMINI_API_KEYS set (will use for enhancement)", flush=True)
        else:
            print(f"[Worker] ⚠️ No NANO_BANANA_API_KEY or GEMINI_API_KEYS — continue mode frame enhancement will be unavailable", flush=True)
    ...
    def _check_kling_variant_queue(self):
        """DISABLED server-side. The official key API only exposes silent Kling
        v2.1, and the Kling-3.0 (audio) web API Cloudflare-blocks datacenter IPs.
        Kling variants are now generated by the LOCAL user-worker via the
        operator's `higgsfield` CLI (see /api/user-worker/clips/kling-pending).
        """
        return
        # --- legacy server-side path (unreachable) ---
        try:
            from config import get_higgsfield_credentials_from_env
            if not get_higgsfield_credentials_from_env():
    ...
    def _check_pending_jobs(self):
        ...
        with get_db() as db:
            ...
            for job in all_pending:
                ...
                if backend_str == 'flow':
                    continue  # Flow jobs handled by user's local worker
    ...
    def _run_job(self, job_id: str):
        ...
        try:
            ...
            # Check if this is an API key failure that should fall back to Flow
            if "No valid API keys" in str(e):
                print(f"[Worker] Job {job_id[:8]}: API keys unavailable — checking Flow fallback...", flush=True)
                try:
                    from backends.selector import is_flow_enabled
                    if is_flow_enabled():
                        with get_db() as db:
                            job = db.query(Job).filter(Job.id == job_id).first()
                            if job:
                                ...
                                # Assign frame keys to clips (API path skips this)
                                # Flow worker needs start_frame/end_frame on each clip
                                try:
                                    ...
                                    use_interp = config_data.get("use_interpolation", True)
                                    ...
                                    if num_images > 0:
                                        ...
                                        for c in clips:
                                            ...
                                            # Start frame
                                            start_fname = uploaded_frames[start_img_idx % num_images] if num_images > 0 else None
                                            
                                            # End frame (simplified scene logic)
                                            end_fname = None
                                            is_last = (idx == total_clips - 1)
                                            if scenes:
                                                cur_scene = None
                                                cur_si = 0
                                                for si, sc in enumerate(scenes):
                                                    if idx in sc.get("clips", []):
                                                        cur_scene = sc
                                                        cur_si = si
                                                        break
                                                if cur_scene:
                                                    sc_clips = sorted(cur_scene.get("clips", []))
                                                    ...
                                                    if is_last_in_scene and not is_last:
                                                        ...
                                                        if clip_mode == "blend":
                                                            end_fname = start_fname
                                            elif num_images == 1 and use_interp and clip_mode == "blend":
                                                end_fname = start_fname
                                            elif num_images > 1 and clip_mode == "blend" and use_interp:
                                                end_fname = start_fname
                                            
                                            c.start_frame = f"jobs/{job_id}/frames/{start_fname}" if start_fname else None
                                            c.end_frame = f"jobs/{job_id}/frames/{end_fname}" if end_fname else None
                                            c.status = "pending"
                                            c.error_message = None
                                        
                                        print(f"[Worker] ✓ Assigned frame keys to {total_clips} clips for Flow redirect", flush=True)
                                    else:
                                        # No frames — just reset clips
                                        clips = db.query(Clip).filter(Clip.job_id == job_id).all()
                                        for c in clips:
                                            c.status = "pending"
                                            c.error_message = None
                                except Exception as frame_err:
                                    print(f"[Worker] ⚠ Frame assignment during redirect failed: {frame_err}", flush=True)
                                    # Still redirect — Flow worker has fallbacks
                                    clips = db.query(Clip).filter(Clip.job_id == job_id).all()
                                    for c in clips:
                                        c.status = "pending"
                                        c.error_message = None
                                ...
                                add_job_log(
                                    db, job_id,
                                    "🔄 API keys unavailable — redirecting to Flow worker automatically",
                                    "WARNING", "system"
                                )
                                db.commit()
                                print(f"[Worker] ✓ Job {job_id[:8]} redirected to Flow backend", flush=True)
                                return  # Don't mark as failed
                except Exception as fallback_err:
                    print(f"[Worker] Flow fallback failed: {fallback_err}", flush=True)
            
            # If we get here, fallback didn't work — treat as normal error
            raise
    ...
    def _resume_waiting_jobs(self):
        ...
        try:
            ...
            if api_keys_config and key_pool:
                status = key_pool.get_pool_status_summary(api_keys_config)
                if status["available"] == 0:
                    # Only log once every 10 minutes to avoid log spam
                    import time as _t
                    now = _t.time()
                    if not hasattr(self, '_last_no_keys_log') or now - self._last_no_keys_log > 600:
                        self._last_no_keys_log = now
    ...
    def _process_clips(
                ...
                # SINGLE IMAGE MODE: Always use same image as end frame for interpolation
                    ...
                    if not scene_transition_handled:
                        ...
                        if is_last_clip and last_frame_index is not None and 0 <= last_frame_index < len(images):
                            ...
                            # Blend mode still needs end frame for visual stability
                            if clip_mode == "blend":
                                use_end_frame = True
                                actual_end_idx = start_idx
                                end_frame_reason = "last clip (storyboard blend: same image for interpolation)"
                            else:
                                use_end_frame = False
                                end_frame_reason = f"last clip (storyboard {clip_mode}), no end frame"
                        elif clip_mode == "blend":
        ...
        def enhance_frame_with_nano_banana(frame_path: Path, original_scene_image: Optional[Path] = None) -> Optional[Path]:
            ...
            Uses the same model/config/prompt as the /api/enhance-frame endpoint
            that the Flow worker calls, ensuring identical results.
            ...
            Returns enhanced path, or frame_path (original) if enhancement failed.
            """
            try:
                ...
                print(f"[EnhanceFrame] google-genai SDK not available: {e}", flush=True)
                return frame_path
            ...
            try:
                ...
                if not api_key:
                    ...
                    if api_keys:
                        api_key = api_keys[0]
                        key_source = "GEMINI_API_KEY pool"
                ...
                print(f"[EnhanceFrame] Enhancing frame: {frame_path.name} ({len(frame_bytes)} bytes)", flush=True)
                ...
                if original_scene_image and original_scene_image.exists():
                    print(f"[EnhanceFrame] Including original scene image for facial consistency: {original_scene_image.name}", flush=True)
                    ...
                    try:
                        ...
                        if "503" in error_str or "overloaded" in error_str.lower() or "429" in error_str:
                            if attempt < max_retries - 1:
                                ...
                                print(f"[EnhanceFrame] ❌ Still overloaded after {max_retries} attempts, using original frame", flush=True)
                                return frame_path
                        else:
                ...
                if not response.candidates:
                    ...
                    print(f"[EnhanceFrame] ❌ No candidates in response. Block reason: {block_reason}", flush=True)
                    return frame_path
                ...
                for p in response.candidates[0].content.parts:
                    if hasattr(p, 'text') and p.text:
                        print(f"[EnhanceFrame] Model text response: {p.text[:200]}", flush=True)
                return frame_path
        ...
        def process_single_clip(clip_index: int):
            ...
            # Log clip mode for visibility
            print(f"[Worker] Clip {clip_index}: mode={clip_mode}, requires_previous={requires_previous}, scene={scene_index}", flush=True)
                ...
                if prev_video:
                    ...
                    if video_exists:
                        ...
                        if extracted:
                            ...
                            was_enhanced = (enhanced != extracted)
                            print(f"[Worker] Clip {clip_index}: Using {'enhanced' if was_enhanced else 'raw extracted'} frame from APPROVED clip {prev_idx}", flush=True)
                            
                            # Log to job for UI visibility
                            with get_db() as log_db:
                                if was_enhanced:
                                    add_job_log(log_db, job_id, f"🖼️ Clip {clip_index + 1}: Frame enhanced via Nano Banana (continue mode)", "INFO", "system")
                                else:
                                    add_job_log(log_db, job_id, f"⚠️ Clip {clip_index + 1}: Enhancement unavailable — using raw extracted frame (continue mode)", "WARNING", "system")
                                log_db.commit()
                        else:
