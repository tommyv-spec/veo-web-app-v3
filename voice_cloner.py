...
def check_openvoice_available() -> dict:
    ...
    try:
        with httpx.Client(timeout=30) as client:
            ...
            # 429 = Modal rate-limiting the health probe, not actually down
            if response.status_code == 429:
                return {
                    "available": True,
                    "message": "OpenVoice endpoint busy (rate limited health check) — will try conversion",
                    "model": MODEL_NAME,
                    "cost": MODEL_COST
                }
            response.raise_for_status()
...
def warmup_openvoice_sync(timeout: int = 120) -> dict:
    ...
    try:
        with httpx.Client(timeout=timeout) as client:
            ...
            if response.status_code == 429:
                elapsed = time.time() - start_time
                logger.info(f"[OpenVoice Warmup] 429 rate limited after {elapsed:.1f}s — endpoint is alive")
                return {
                    "available": True,
                    "message": f"OpenVoice alive (rate limited) after {elapsed:.1f}s",
                    "model": MODEL_NAME,
                    "warmup_time": elapsed
                }
            response.raise_for_status()
