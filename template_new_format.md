...
- **line:** 
- **veo_model:** Omni Flash      # v961 — OPTIONAL per-clip render model. Absent = the job-level model (every pre-v961 build). Exact, case-sensitive, one of: Omni Flash | Veo 3.1 - Quality | Veo 3.1 - Fast | Veo 3.1 - Lite | Veo 3.1 - Lite [Lower Priority] (the last two are DIFFERENT dropdown options, not aliases). Attaches to the `- **line:**` above it, same as clip_duration_s, so one scene can hold two models. A build that MIXES models needs a `RENDER MODELS:` line in §0 or the auditor WARNs. v943 charswap / v959 movie-section clips force Omni and a conflicting declaration HARD-FAILS at import.
